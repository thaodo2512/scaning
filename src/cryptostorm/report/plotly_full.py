...
def build_reports(cfg: Mapping[str, Any], eff: EffectiveConfig, *, data_root: Path, artifacts_root: Path, out_root: Path, include_oi_liq: bool = False) -> None:
    ...
    try:
        ...
        lines = [
            ...
            "<title>CryptoStorm Plotly Reports</title>",
            ...
            f"<h2>CryptoStorm Reports (Plotly) <span class=ts>(generated {now})</span></h2>",
            "<ul>",
        ]
        for s, _m in items:
            lines.append(f"<li><a href=\"{s}_plotly.html\">{s}</a></li>")
