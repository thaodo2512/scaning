...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    p_ret.add_argument("--poll-offset-s", type=float, default=60.0)
    ...
    p_tel.add_argument("--debug", action="store_true")
    ...
    p_rt.add_argument("--poll-offset-s", type=float, default=60.0)
    ...
    if args.cmd == "audit":
        argv = [args.config, "--data", args.data, "--min-ratio", str(args.min_ratio)]
        if args.debug:
            argv.append("--debug")
    ...
    if args.cmd == "feature":
        argv = [args.config, "--data", args.data, "--out", args.out, "--log-level", args.log_level, "--interval", args.interval]
        if args.update_last:
            argv.append("--update-last")
        if int(args.workers) > 1:
            argv += ["--workers", str(int(args.workers))]
        return feature_main(argv)
